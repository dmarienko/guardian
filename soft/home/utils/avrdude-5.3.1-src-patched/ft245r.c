...
static unsigned char ft245r_gates;
...
static void ft245r_set_bitclock(PROGRAMMER * pgm) {
  ...
  if(r!=FT_OK)	fprintf(stderr," ft245r: error setting baud rate\n");
...
retry:
  ...
  printf("RES 0 : %02x%02x%02x%02x --> %d\n",res[0],res[1],res[2],res[3], reset_ok);
  for (i=0; i<3; i++) {
     cmd[0] = 0x30;
     cmd[1] = 0;
     cmd[2] = i;
